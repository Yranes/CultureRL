#!/bin/bash
#SBATCH -J usaSearchAblation                       
#SBATCH -o grpo_ablation_usa.out                           
#SBATCH -p compute 
#SBATCH -N 1  
#SBATCH -t 12:00:00
...
YAML_PATH="/home/hzli/code/WVS/open-r1/recipes/wvs_value/ablation_search/ablation_usa_full.yaml"

RUN_DIR_BASE='/home/hzli/code/WVS/open-r1/runs'
RUN_NAME="SearchRule_train_qwen2.5_usa_value_${SUFFIX}"
...
CUDA_VISIBLE_DEVICES=0,1 ACCELERATE_LOG_LEVEL=info\
    ...
    --run_name $RUN_NAME \
    --learning_rate 5e-6\
    --system_prompt "You are a real person with an American cultural background. Please answer the questions honestly based on your personal value system rooted in American culture. Your response must reflect a clear and decisive stance—do not offer multiple perspectives or qualify your answer with exceptions. Avoid saying “However”, “both sides are valid”, or providing a nuanced analysis. Give one straightforward, firm answer that aligns with traditional American values."\
    2>&1 | tee "$LOG_PATH"

#     --dataset_name /home/hzli/code/WVS/open-r1/data/0710_5/filter/train.json \