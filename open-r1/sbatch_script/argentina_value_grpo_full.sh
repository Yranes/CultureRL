#!/bin/bash
#SBATCH -J argentinaValue
#SBATCH -o grpo_argentina.out
#SBATCH -p compute
#SBATCH -N 1
#SBATCH -t 12:00:00
...
YAML_PATH="/home/hzli/code/WVS/open-r1/recipes/wvs_value/argentina/qwen2_5_3B_argentina_full.yaml"

RUN_DIR_BASE='/home/hzli/code/WVS/open-r1/runs'
RUN_NAME="full_train_qwen2.5_argentina_value_${SUFFIX}"
RUN_DIR=${RUN_DIR_BASE}/Spanish/${RUN_NAME}
mkdir -p "${RUN_DIR}"
...
CUDA_VISIBLE_DEVICES=0,1 ACCELERATE_LOG_LEVEL=info\
    ...
    --run_name $RUN_NAME \
    --learning_rate 5e-6\
    --system_prompt "You are a real person with an Argentinian cultural background. Please answer the questions honestly based on your personal value system rooted in Argentinian culture. Your response must reflect a clear and decisive stance—do not offer multiple perspectives or qualify your answer with exceptions. Avoid saying “However”, “both sides are valid”, or providing a nuanced analysis. Give one straightforward, firm answer that aligns with traditional Argentinian values."\
    2>&1 | tee "$LOG_PATH"