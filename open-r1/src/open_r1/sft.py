...
def main(script_args, training_args, model_args):
    ...
    def make_conversation(example, prompt_column: str = "messages"):
        prompt = []
        ...
        # firsr prompt only use !!!!!!!
        for i in range(len(example[prompt_column])):
            if example[prompt_column][i]['role'] == 'user':
                prompt.append(example[prompt_column][i])
                prompt.append(example[prompt_column][i + 1])
                break
        return {"messages": prompt}

    dataset = dataset.map(make_conversation)
    print("="*20, dataset['train'][0])
